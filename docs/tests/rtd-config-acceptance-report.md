...
| Description | Progress & validation report for the RTD CfgFile CLI minimal system (roadmap stage 1). Records the proven Uart reference path (deterministic tests, static checks, and real S32DS headless vendor validation) and the remaining work under the all-modules-validated acceptance bar. Note: §3 lists the historical implemented `RTD-M1-MIN-*` Uart suite; the forward acceptance target is the E2E scheme `RTD-MEX-*` in `rtd-config-test-cases.md`. |

## 1. Status — minimal system IN PROGRESS (bar: all seven modules validated)

The acceptance bar is **module parity across the minimal system** (test
strategy; E2E cases in `rtd-config-test-cases.md`): the minimal system is
accepted only when **all seven modules** (Mcu, BaseNXP, Platform, Port, Dio,
Mcl, Uart) pass the gate — not Uart alone. Under that bar **the minimal system
is in progress, not yet accepted.** This report records the proven reference
path and the verified vendor flow that the remaining modules replicate.

**Reference path PROVEN and vendor-validated.** The Uart path — LPUART and FlexIO
channels in interrupt (IRQ) mode — is fully delivered through a deterministic
CLI/JSON contract with narrow, byte-faithful, owned `.mex` edits, and the tool's
own `validate` returns ConfigTools **exit 0 with zero SEVERE `[TOOL]` problems**
on both configured projects (§6). This establishes the architecture, the write
path, and the verified S32DS flow now reused by the other modules.

**Cross-cutting commands done:** `inspect`, `pin-options` (the pin data itself
must be rebuilt from the Excel — domain-truth §1).

**Remaining for the minimal system:** bring Mcu, BaseNXP, Platform, Port, Dio,
Mcl each to the same configure + S32DS-validated bar as Uart
(`rtd-config-test-cases.md`: the `RTD-MEX-*` E2E cases, isolated protocol),
implement the Uart/Mcl DMA capability (`RTD-MEX-UART-003`), and rebuild
`pins.json` from the pin-mux workbook.
...
Performed per `rtd-config-subagent-validation.md`. Four cold, context-isolated
...
from `rtd-config-legacy-skills-experience.md` are implemented and covered: XML
...
- **Documentation boundary:** `docs/OBSOLETE_NEVER_TOUCH!!!/` was treated as a
...
- Source: `autombd-rtd/rtd-config-cli-py/rtd_config/` (CLI, S32 `.mex` backend, modules, checks, resources).
- Tests: `tests/unit` (30), `tests/integration` (7), `tests/e2e` (7).
- Assets: `autombd-rtd/assets/nxp/s32k3/port/pins.json`.
- Companion skill: `autombd-rtd/SKILL.md`.
- Validation experience: `docs/references/rtd-config-legacy-skills-experience.md`.
...
| 2026-06-03 | 0.3.0 | Reframed to "in progress" under the raised seven-module-parity bar (test-strategy v0.5.0): Uart reference path proven and vendor-validated; remaining six modules + pin-data rebuild tracked as the forward parity matrix. |
| 2026-06-10 | 0.4.0 | Fourth-round review alignment: renamed from `rtd-config-m1-acceptance-report.md`; forward target re-keyed to the E2E scheme `RTD-MEX-*` in `rtd-config-test-cases.md` (incl. the DMA case as a tracked capability gap); asset/reference paths updated (`autombd-rtd/assets/`, references/). |
