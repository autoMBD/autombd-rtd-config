...
| 2026-06-17 | 0.19.0 | RTD-MEX-PLATFORM-001 KPI baseline recorded **MISS** (issue #14 baseline). Functional **PASS** re-confirmed under TRUE black-box protocol — independent vendor gate: exit 0, 120 files, 0 SEVERE; `.mex` diff: exactly `IsrPriority` 0→2 on `LPUART3_IRQn`, enabled with `LPUART_UART_IP_3_IRQHandler`, FLEXIO entry untouched. **KPI: MISS** — 1 edit attempt (meets expectation), but `kpi_seconds` 73.3 s > 1 min budget. Session evidence: `session_id 019ed61a-55a9-73b3-9b78-673b474420bd`, 11 agent commands, the overhead is intent-analysis + planning + redundant `inspect`/`--help`/dry-run → optimization follows. |
| 2026-06-17 | 0.20.0 | RTD-MEX-PLATFORM-001 KPI re-measured **PASS** after replacing per-module one-shot recipes with a universal one-shot workflow (issue #14 owner review). **Optimization:** `autombd-rtd/SKILL.md` "plan → configure → validate" section now presents a **universal** one-shot pattern — a prominent, imperative checklist (no `inspect`, no `--help`, no dry-run, no duplicate `validate`) that applies to any module equally — instead of per-module "One-shot recipe:" entries with hardcoded example values that would not scale beyond the known test cases. The BaseNXP per-module recipe was also removed (the `--enable-system-timer` example is kept as a normal flag reference). **Re-measured:** baseline 73.3 s (MISS) → **58.7 s ≤ 1 min (PASS)**; agent followed the universal pattern: SKILL.md read → `set --configure` → `check` → `validate` (4 commands, no exploration). Functional PASS unchanged (independent vendor gate exit 0, 120 files, 0 SEVERE; `.mex` IsrPriority 0→2 on LPUART3_IRQn only). Session: `019ed63a-0699-7b72-8768-7a64a68e504a`. |
