...
4. **Isolated E2E acceptance cases** — the cases in `rtd-config-test-cases.md`,
   ...
   successful code generation. The Tester also records the case's KPI result.
...
3. **Tester** runs the gate: the deterministic suite, vendor validation, and the
   ...
   SEVERE count and KPI evidence. **E2E execution is context-isolated** (§2.4);
   the isolation mechanism is whatever the agent platform provides (a fresh,
   non-inherited context) — context isolation is the requirement, not any
   specific parameter.
...
run exceeding 10 min triggers orchestrator intervention and issue capture. KPI
evidence must include the measured elapsed time, whether the case used one edit
attempt or required rework, the optimization-iteration count, and the final
KPI status (`pass`, `miss`, or `miss-after-3`). E2E subagent prompts live in the
case table (`rtd-config-test-cases.md`); the executed Uart-reference black-box
round is recorded in `rtd-config-subagent-validation.md`.
