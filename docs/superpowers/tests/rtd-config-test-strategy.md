...
| RTD-M1-MIN-002 | — | **Reserved / removed from M1** — RTD 7.0.1 models the Uart asynchronous method as interrupt or DMA only; there is no polling value, so "polling mode" is not a configurable `.mex` scenario (blocking/polling is an application-level driver-call pattern). | — | — | Not part of the M1 mandatory gate. Interrupt coverage is RTD-M1-MIN-003. | — |
...
| RTD-M1-MIN-004 | — | **Reserved / removed from M1** — same reason as MIN-002: polling is not an RTD 7.0.1 `.mex` async-method value. | — | — | Not part of the M1 mandatory gate. FlexIO interrupt coverage is RTD-M1-MIN-005. | — |
