...
> `docs/roadmaps/rtd-config-roadmap.md` with the user's inline REVIEW comments preserved for traceability.
...
<!-- REVIEW: 需要重构Roadmap，文档简化，只描述基本路线图即可，无需说明技术细节。

Milestone如下：

1 最小系统模块`Mcu`, `BaseNXP`, `Platform`, `Port`, `Dio`, `Mcl`, and `Uart` on NXP S32K3 for mex file
2 补全剩下的其他模块 for mex
3 添加缺省的配置模块，或从零创建.mex配置文件
4 支持 EB Tresos 和 .xdm
5 长期计划：支持FreeRTOS、RTD Stacks、RTD CDDs，支持更多芯片系列 -->
