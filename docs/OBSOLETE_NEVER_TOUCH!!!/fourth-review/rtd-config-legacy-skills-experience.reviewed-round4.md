...
> `docs/specs/rtd-config-legacy-skills-experience.md` with the user's inline REVIEW comments preserved for traceability.
...
# RTD CfgFile CLI M1 Legacy Skills Experience
...
<!-- REVIEW: 这个文件rtd-config-legacy-skills-experience.md因该放到 references/目录下 -->
...
validation results as the authority for Milestone 1 implementation.
...
The Milestone 1 extraction covers these deprecated skills and references:

| Current M1 module | Deprecated source directory |
| --- | --- |
...
| Missing modules | Milestone 1 works on existing complete fixture projects. Creation from seeds is reserved for later milestones, but M1 implementation must not make seed handling impossible. |
...
| Module | Owned configuration | Key M1 experience |
| --- | --- | --- |
...
| `Mcl` | Mcl common resources: FlexIO, DMA, eMIOS, TRGMUX, LCU, cache gates | Mcl is a dependency owner. For M1, FlexIO common resources are owned by Mcl and consumed by Uart. Feature switches and arrays must match; enabling FlexIO common without coherent logic channels is incomplete. DMA stays deferred. |
| `Uart` | Uart channels, Uart general settings, LPUART/FlexIO channel containers and Uart-side refs | LPUART and FlexIO paths are different. Uart owns channel settings and references; Mcu owns clock references, Mcl owns FlexIO logic channels, Port owns TX/RX pin routing, Platform owns IRQ entries. DMA is outside M1. |

## Mcu Notes

Mcu is the clock and mode foundation for the Uart minimum stack.

- M1 should support only minimum clock/reference and peripheral-clock behavior
...
- M1 should detect and preserve existing BaseNXP/OsIf configuration needed by
...
- MPU, MCM, and INTM remain advanced or future work for M1 unless needed by the
  existing fixture.
...
Dio is simple in M1 but still needs clear ownership.
...
- Multi-partition and virtual-wrapper behavior requires existing Rm/EcuC
  references and is not part of M1 minimum configuration.

## Mcl Notes

Mcl is the key dependency module for FlexIO-backed Uart in M1.
...
- M1 must reject or defer DMA instead of partially configuring it.

## Uart Notes

Uart is the M1 user-facing driver path.

- M1 supports LPUART and FlexIO-backed Uart in interrupt (IRQ) mode; polling is
  not an RTD 7.0.1 async-method value, and DMA is deferred.
...
## M1 Implementation Impact

The Milestone 1 execution plan must use these experience-derived checks:
...
- static checks for stale FlexIO `UartHwChannelRef`, duplicate LPUART hardware
  channels, duplicate IDs/names, missing Mcl FlexIO logic channels, and M1 DMA
  rejection;
