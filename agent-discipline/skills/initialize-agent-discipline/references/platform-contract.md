...
Use a deterministic deployment script with separate renderers for Claude Code,
OpenCode, and Codex. Parse each canonical file once into `name`, `description`,
`tools`, `model`, and Markdown body. Reject malformed frontmatter, duplicate
fields, unsupported fields, and unknown tools before writing any target.
