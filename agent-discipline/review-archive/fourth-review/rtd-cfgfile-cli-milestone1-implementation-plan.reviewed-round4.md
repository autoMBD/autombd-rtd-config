...
> `docs/plans/rtd-cfgfile-cli-milestone1-implementation-plan.md` with the user's inline REVIEW comments preserved for traceability.
...
<!-- REVIEW: 该文件去掉所有和milestone 1相关的描述，只保留项目implementation相关信息，计划就是一个模块一个模块的加，最开始的七个是最小系统，所以是一起完成，但在这七个模块之后，我会给新的指令和任务添加新模块，所有的模块都在相同的框架下开发和测试。所有文档名称中的m1也要去掉，仅在rtd-config-roadmap.md描述不同阶段的任务 -->

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

> **Status (2026-06-03) — partially superseded.** This was the original
> Uart-first execution plan and is retained as a historical record. The current
> scope/acceptance is **seven-module parity** under `rtd-config-test-strategy.md`
> v0.5.0, with domain facts in `rtd-config-domain-truth.md`. The polling cases
> referenced below (`RTD-M1-MIN-002` / `RTD-M1-MIN-004`) are **withdrawn** — RTD
> 7.0.1 has no Uart polling async-method value. Do not treat the embedded code
> blocks as the implementation source of truth (the live code is authoritative).
>
> **Repository layout (current).** The paths below predate the repository
> restructure. Remap them to the current layout — authoritative in
> `rtd-config-core-design.md` (Repository layout) and the root `README.md`:
> `rtd_config/` → `autombd-rtd/rtd-config-cli-py/rtd_config/`;
> `data/...` → `autombd-rtd/data/nxp/<family>/<module>/`;
> `fixtures/...` → `tests/fixtures/nxp/<ds|eb>/<family>/<project>/`;
> `skills/rtd-config/SKILL.md` → `autombd-rtd/SKILL.md`; and the family
> token `s32k3xx` → `s32k3`.
...
Milestone 1 implements only the mandatory minimum test set from `docs/tests/rtd-config-test-strategy.md`:
...
- `.mex` document editing and M1 provider implementation must use and comply
  with `docs/specs/rtd-config-legacy-skills-experience.md`.
...
**Files:**
- Create: `data/nxp/s32k3/port/pins.json`
...
**Files:**
- Create: `docs/tests/rtd-config-m1-subagent-validation.md`
...
```powershell
rg "implementation notes|main-agent|hidden assumptions|debugging process" docs/tests/rtd-config-m1-subagent-validation.md
```
...
```powershell
git add docs/tests/rtd-config-m1-subagent-validation.md
...
- [ ] Confirm `.mex` quick-selection behavior from
      `rtd-config-legacy-skills-experience.md` is covered by unit or fixture
      tests.
