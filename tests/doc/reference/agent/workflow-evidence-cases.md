...
| WE08 | R04,R08,R14 | Registered support repair before a Candidate, or after an existing execution and before/after its retest. | Original case approval stays anchored; pending support changes effective Test only, while executed Test and Candidate remain old until the source-bound retest advances them without a correction. |
...
| WE11 | R06 | Task introduces ignored-state, temporary overlay, Reviewer lessons, or a bound command-result/lesson/disclosure-review attachment at an ordinary path into the Candidate tree. | Reject the exact prohibited or bound evidence content; ordinary unbound fixture/reference names stay legal. |
...
| WE26 | R10,R12,R16 | CLI uses an explicitly injected executable transport for owned remote GET proof, including command timeout. | Only repository-relative authenticated GitHub GET argv, explicit hostname and deadline; exact HTTP exit mapping; timeout returns 124 with safe COMMAND_TIMEOUT on stderr and empty stdout. |
...
| 2026-09-09 | 0.1.1 | Completed three existing K1 branches identified by pre-Gate Test spec QA: owned GET timeout, pending-support execution identity and ordinary-path bound attachments. |
